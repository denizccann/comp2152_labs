...
def record_attempt(username, success):
    """Record one login attempt to the database."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    attempt_date = str(datetime.datetime.now())
    cursor.execute(
        "INSERT INTO login_attempts (username, success, attempt_date) VALUES (?, ?, ?)",
        (username, success, attempt_date)
    )
    conn.commit()
    conn.close()
...
def get_failed_attempts(username):
    """Get all failed attempts for a specific user."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
def count_failures_per_user():
    """Count failed attempts grouped by user."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
def delete_old_attempts(username):
    """Clear all attempts for a user and return the number of deleted rows."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
