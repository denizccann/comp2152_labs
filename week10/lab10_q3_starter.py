...
def get_events_by_severity(severity):
    """Get all events matching a specific severity level."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
def get_recent_events(limit):
    """Get the most recent events up to a specified limit."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
def count_by_severity():
    """Count events grouped by severity, ordered by count descending."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
def safe_query(query):
    """Run a custom query safely, catching any SQLite errors."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    try:
